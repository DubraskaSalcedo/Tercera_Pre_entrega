...
from AppProyectoFinal.models import Estudiante, Profesor, Curso4, Comentario, Taller
    ...
    if request.method == "POST":
        ...
        if mi_formulario.is_valid():
            ...
            if informacion['image']==None:
                ...
                get_curso.body = informacion['image']
                get_curso.save()
            return redirect("AppProyectoFinalListaTalleres")
...
def mostrar_formulario_de_buscar_curso(request):
    if request.method == "GET":
        ...
        if mi_formulario.is_valid():
            informacion = mi_formulario.cleaned_data
            cursos_filtrados = Curso4.objects.filter(title__icontains=informacion['title'])
            context = {"cursos": cursos_filtrados}
...
def eliminar_curso(request, title):
    get_curso= Curso4.objects.get(title=title)
    get_curso.delete()
...
def mostrar_detalles_de_curso(request, title):
    get_curso = Curso4.objects.get(title=title)
    all_comments = Comentario.objects.filter(curso4=get_curso)
    if request.method == "POST":
        form = ComentarioForm(request.POST)
        if form.is_valid():
            informacion = form.cleaned_data
            nuevo_comentario = Comentario(curso4=get_curso, cuerpo=informacion['cuerpo'],
